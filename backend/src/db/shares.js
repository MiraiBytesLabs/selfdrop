...
export const createShare = db.transaction((params) => {
  const { uuid, filePaths, expiresAt, downloadLimit, passwordHash, maskFilenames, name } = params;
  ...
  const insertFile = db.prepare('INSERT INTO share_files (share_id, file_path) VALUES (@shareId, @filePath)');
...
export function listShares() {
  return db.prepare('SELECT * FROM shares ORDER BY created_at DESC').all().map(attachFiles);
}

export function incrementDownloadCount(uuid) {
  db.prepare('UPDATE shares SET download_count = download_count + 1 WHERE uuid = ?').run(uuid);
}
...
export function validateShare(uuid) {
  ...
  if (new Date() > new Date(share.expires_at)) return { valid: false, reason: 'expired' };
