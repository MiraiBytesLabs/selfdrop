...
function verifyFilePaths(filePaths) {
  for (const filePath of filePaths) {
    let resolved;
    try { resolved = resolveSafePath(filePath); }
...
function formatShare(share) {
  ...
  const isLimitReached = share.download_limit !== null && share.download_count >= share.download_limit;
  return {
    uuid:          share.uuid,
...
router.post('/', async (req, res) => {
  ...
  try {
    ...
    const share = sharesDb.createShare({
      uuid: generateToken(), filePaths: data.filePaths, expiresAt: data.expiresAt,
      downloadLimit: data.downloadLimit, maskFilenames: data.maskFilenames,
...
router.delete('/:uuid', (req, res) => {
  try {
    if (!sharesDb.deleteShare(req.params.uuid)) return res.status(404).json({ error: 'Share not found.' });
